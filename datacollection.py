...
NEWSAPI_KEY = '9026d49691d14f95bb9320730158b797'
...
def fetch_live_data(keyword):
    ...
    two_days_ago = datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=7)
    
    params = {
        'apiKey': NEWSAPI_KEY,
        'q': keyword,
        'from': two_days_ago.strftime('%Y-%m-%d'),  # From 1 days ago
        'to': datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d'),  # To today
        'language': 'en',
    }
...
def identify_disaster_event(title):
    ...
    for keywords in disaster_keywords:
        if keywords.lower() in title.lower():
            return keywords.capitalize()
...
if __name__ == "__main__":
    ...
    for keyword in disaster_keywords:
        ...
        for article in live_data:
            published_at = article.get('publishedAt', datetime.datetime.now(datetime.UTC))
    ...
    uri = "mongodb+srv://samarmittal59:Qwerty%40123@cluster0.raeompg.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
